...
import com.samsung.hms.support.sms.ReadSmsManager
import com.samsung.hms.support.sms.common.ReadSmsConstant

import com.utsabb.sms_demo.databinding.ActivityMainBinding
import com.utsabb.sms_demo.sms.samsungOtpBroadcastReceiver
import com.utsabb.sms_demo.sms.SmsHelper


/**
 * samsung SMS retriever demo
...
class MainActivity : AppCompatActivity() {
     ...
     * the broadcast received here is sent from [samsungOtpBroadcastReceiver.onReceive]
     */
    private val updateUIReceiver = object : BroadcastReceiver() {
        override fun onReceive(
            ...
            intent.getStringExtra(samsungOtpBroadcastReceiver.DEMO_EXTRA_SMS)?.let {
    ...
    /**
     * samsung otp broadcast receiver
     ...
     */
    private val  samsungOtpBroadcastReceiver = samsungOtpBroadcastReceiver()
    /*** receivers : end **/
    ...
    private fun registerSmsBroadcastReceiver() {
        ...
        registerReceiver(samsungOtpBroadcastReceiver, intentFilter)
    }

    /**
     * local receiver that will receive the OTP from [samsungOtpBroadcastReceiver] to update the UI
     *
     */
    private fun registerLocalOtpBroadcastReceiver() {
        setStatus("registerOtpBroadcastReceiver:invoke")
        val filter = IntentFilter()
        filter.addAction( samsungOtpBroadcastReceiver.DEMO_ACTION_UPDATE_UI )

        registerReceiver(updateUIReceiver, filter)
    }
    override fun onStop() {
        super.onStop()
        unregisterReceiver(samsungOtpBroadcastReceiver)
    ...
    /**
     * initialize samsung sms reader, register Otp receiver, then send otp using user's sim card for demo purposes
    ...
    private fun initSmsReaderThenSendOtp() {
        ...
        initsamsungSmsReader()
        sendOtpSms()
    }

    /**
     * initialize samsung sms reader, required for [samsungOtpBroadcastReceiver] to receive OTP SMS
     * called in [initsamsungSmsReader]
     */
    private fun initsamsungSmsReader() {
        ...
        task.addOnCompleteListener {
            ...
            if (task.isSuccessful) {
                ...
                setStatus("samsung ReadSmsManager service has been enabled.")
            } else {
                setStatus("samsung ReadSmsManager service failed to initialized")
